...
module.exports = class extends Argument {
	...
	async _get_role(message, search) {
		...
		instances.push((command.guildOnlyArgs && Array.isArray(command.guildOnlyArgs) && command.guildOnlyArgs.length && (command.guildOnlyArgs.includes('role') || command.guildOnlyArgs.includes('roles'))) ? await message.guild.roles.fetch() : new RoleStore(message.channel, this.client.guilds.map((guild) => guild.roles.array()).flat()));

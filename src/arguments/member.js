...
module.exports = class extends Argument {
	...
	async _get_member(message, search) {
		...
		const member_store = new Map(...instances.filter((store) => [`${instances_type ? 'Klasa' : ''}GuildMemberStore`].includes(new Type(store).is)));
		member_store
		...
		return items.filter((item) => !['KlasaGuildMemberStore', 'GuildMemberStore'].includes(new Type(item).is));
	}
	
	async _displayInstances(message, instances) {

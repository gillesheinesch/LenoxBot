...
module.exports = class extends Monitor {
	...
	async run(message) {
		if (message.guild && !message.guild.me) await message.guild.members.fetch(this.client.user, true);
		if (!message.channel.postable) return undefined;
		if (this.mentionOnly.test(message.content)) return message.send(null, {
				embed: {
					color: 0x3669FA,
					title: `Hi!`,
					description: `I'm **${this.client.user.tag}**!\n\nRun **${message.guild ? message.guild.settings.prefix : '@' + this.client.user.tag + ' '}help** to see what commands I have!`,
					thumbnail: {

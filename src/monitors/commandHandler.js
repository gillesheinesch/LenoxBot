			...
			try {
				...
				try {
					...
					if ((message.channel.type === 'dm' && message.author.id !== process.env.BOT_OWNER_ID) || (message.channel.type !== 'dm' && message.channel.id !== '345551930459684866')) {
						if (this.client.channels.has(process.env.BOT_DEBUG_MESSAGES)) {
							this.client.channels.get(process.env.BOT_DEBUG_MESSAGES).send(null, {
								embed: {
									color: 15684432,
									timestamp: new Date(),
									description: `\`\`\`\n${message.content}\n\`\`\``,
					...
					if (this.client.channels.has(process.env.BOT_CONSOLE_LOG)) {
						this.client.channels.get(process.env.BOT_CONSOLE_LOG).send(null, {
							embed: {
								color: 15684432,
								timestamp: new Date(),

...
exports.run = async (client, msg, args, lang) => {
	...
	await sql.get(`SELECT * FROM scores WHERE guildId ="${msg.guild.id}" AND userId ="${userId}"`).then(row => {
		if (row) {
			...
			sql.run('INSERT INTO scores (guildId, userId, points, level) VALUES (?, ?, ?, ?)', [msg.guild.id, userId, 0, 0]);
		}
	}).catch(() => {
...
exports.help = {
	name: 'removexp',
	description: '',
	usage: 'removexp {userid} {amount}',
	example: ['removexp 327533963923161090 1000'],
	category: 'botowner',

...
exports.run = async (client, msg, args, lang) => {
	...
	var current = lang.weather_current.replace('%location', weatherInfo.location.city).replace('%temperature', `${weatherInfo.item.condition.temp}°F/${celsius(weatherInfo.item.condition.temp)}°C`);
	var temperature = lang.weather_temperature.replace('%highesttemperature', `${forecast.high}°F/${celsius(forecast.high)}°C`).replace('%lowesttemperature', `${forecast.low}°F/${celsius(forecast.low)}°C`);
	var queryby = lang.weather_queryby.replace('%authortag', msg.author.tag);

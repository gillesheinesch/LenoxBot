...
exports.run = async(client, msg, args, lang) => {
	...
	const validation = ['administration', 'help', 'music', 'fun', 'searches', 'nsfw', 'utility', 'botowner', 'moderation', 'trello', 'staff', 'application', 'currency', 'partner'];
	...
	for (i = 0; i < margs.length; i++) {
		if (validation.indexOf(margs[i].toLowerCase()) >= 0) {
			if (margs[1].toLowerCase() == "administration") {
				...
				const message = await msg.channel.send(`${client.commands.filter(c => c.help.category === "currency").array().slice(0, 20).map(cmd => `${tableload.prefix}${cmd.help.name}${' '.repeat(longest - cmd.help.name.length)} :: ${lang[`${cmd.help.name}_description`]}`).join("\n")}`, { code:'asciidoc' });
								...
								var collector = message.createReactionCollector((reaction, user) => user.id === msg.author.id, { time: 30000 });
								collector.on('collect', r => {
									var reactionadd = client.commands.filter(c => c.help.category === "currency").array().slice(first + 20, second + 20).length;
									var reactionremove = client.commands.filter(c => c.help.category === "currency").array().slice(first - 20, second - 20).length;
				
									if (r.emoji.name === '▶' && reactionadd !== 0) {
										r.remove(msg.author.id);
										const array = client.commands.filter(c => c.help.category === "currency").array();
										...
										r.remove(msg.author.id);
										const xxx = client.commands.filter(c => c.help.category === "currency").array();
				...
				const message = await msg.channel.send(`${client.commands.filter(c => c.help.category === "partner").array().slice(0, 20).map(cmd => `${tableload.prefix}${cmd.help.name}${' '.repeat(longest - cmd.help.name.length)} :: ${lang[`${cmd.help.name}_description`]}`).join("\n")}`, { code:'asciidoc' });
								...
								var collector = message.createReactionCollector((reaction, user) => user.id === msg.author.id, { time: 30000 });
								collector.on('collect', r => {
									var reactionadd = client.commands.filter(c => c.help.category === "partner").array().slice(first + 20, second + 20).length;
									var reactionremove = client.commands.filter(c => c.help.category === "partner").array().slice(first - 20, second - 20).length;
				
									if (r.emoji.name === '▶' && reactionadd !== 0) {
										r.remove(msg.author.id);
										const array = client.commands.filter(c => c.help.category === "partner").array();
										...
										r.remove(msg.author.id);
										const xxx = client.commands.filter(c => c.help.category === "partner").array();
